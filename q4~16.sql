...
select * from tottenham_squad where p_position = '공격수'; #5
delete from tottenham_squad where p_number = 7; #6
update tottenham_squad set p_name = '로드리고밴탄쿠르' where p_number=30; #7
select * from tottenham_squad where p_number >= 7; #8~
select * from tottenham_squad where p_name = '로드리고밴탄쿠르';
select id,p_name from tottenham_squad where p_name = '로드리고밴탄쿠르'; #~8
select * from tottenham_squad where p_name like '%제%' and p_number > 6; #9
ALTER TABLE tottenham_squad ADD weekly_pay int; #10
select * from tottenham_squad order by p_number; #11~
select * from tottenham_squad order by p_number asc;
select * from tottenham_squad order by p_number desc; 
select * from tottenham_squad order by p_height desc, p_weight; #~11
update tottenham_squad set weekly_pay = 0 ; #12~
update tottenham_squad set weekly_pay = 1000 where p_number=9; 
select * from tottenham_squad order by weekly_pay desc; #~12
select p_number as 등번호,p_name as 선수명 from tottenham_squad; #16
...
insert into tottenham_squad values(0,7,'손흥민','1992-07-08','공격수',183,78,1);
